...
if not TAVILY_API_KEY:
    raise RuntimeError("TAVILY_API_KEY is missing in environment variables")
...
def hospital_search(query: str) -> str:
    """
    Search hospitals, clinics, emergency centers using Tavily.
    ...
    try:
        ...
        if not results:
            return "No hospitals found for this query."

        output = []
        for i, r in enumerate(results, start=1):
            name = r.get("title", "Unknown Hospital")
            url = r.get("url", "")
